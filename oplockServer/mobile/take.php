...
if($userdata['status'] != 1 && !isset($_POST['submit'])){
	echo("0");
	exit();
}else{
	...
	if($appData['status'] == 1)
		if(mysqli_query($link, "UPDATE `applications` SET `status` = '2', `taken` = '" . intval($userdata['id']) . "' WHERE `applications`.`id` = " . intval($_POST['id']))){
			echo("1");
			exit();
		}
}
